...
class CNNMINE2X_dropout(nn.Module):
    def __init__(self, n, k, w1=8, kernel=2,  dropoutfc=None, dropoutconv=None, input_size=1, initializations=None):
        super().__init__()
        self.conv1 = nn.Conv3d(input_size, n, kernel_size=kernel, stride=1)
        self.conv2 = nn.Conv3d(n, int(k*n), kernel_size=kernel, stride=1)
        self.fc1 = nn.Linear(int(k*n), n)
        self.fc2 = nn.Linear(n, 1)
        self.relu = nn.LeakyReLU()
        self.adaptivemaxpool3d_2 = nn.AdaptiveMaxPool3d(w1)
        self.adaptivemaxpool3d_1 = nn.AdaptiveMaxPool3d(1)
...
class CNNMINE5X_dropout(nn.Module):
    def __init__(self, n=20, k=2.5, w1=32, w2=16, w3=8, w4=4, w5=2, dropoutfc=0.3, dropoutconv=0.15, input_size=1, initializations=None):
        super().__init__()
        self.conv1 = nn.Conv3d(input_size, n, kernel_size=5)
        self.conv2 = nn.Conv3d(n, int(n * k), kernel_size=4)
        self.conv3 = nn.Conv3d(int(n * k), int(n * k * k), kernel_size=3)
        self.conv4 = nn.Conv3d(int(n * k * k), int(n * k * k * k // 2), kernel_size=3)
        self.conv5 = nn.Conv3d(int(n * k * k * k // 2), int(n * k * k * k // 2), kernel_size=2)
        ...
        self.adaptivemaxpool3d_5 = nn.AdaptiveMaxPool3d(w5)

        self.fc1 = nn.Linear(int(n * k * k * k // 2) * w5**3, int(n * k * 2))  
        self.fc2 = nn.Linear(int(n * k * 2), int(n * k))
        self.fc3 = nn.Linear(int(n * k), int(n * k // 2))
        self.fc4 = nn.Linear(int(n * k // 2), 1)
        ...
        if initializations is not None:
            ...
            if initializations == 'xavier':
                ...
                initializer = torch.nn.init.kaiming_uniform_
            if initializer:
                for layer in [self.conv1, self.conv2, self.conv3, self.conv4, self.conv5, self.fc1, self.fc2, self.fc3, self.fc4]:
                    initializer(layer.weight)

    def forward(self, x):
        # Define checkpointed blocks as lambdas to include intermediate activations
        # out = checkpoint(lambda x: self.adaptivemaxpool3d_1(self.convdropout(self.relu(self.conv1(x)))), x)
        # out = checkpoint(lambda x: self.adaptivemaxpool3d_2(self.convdropout(self.relu(self.conv2(x)))), out)
        # out = checkpoint(lambda x: self.adaptivemaxpool3d_3(self.convdropout(self.relu(self.conv3(x)))), out)
        # out = checkpoint(lambda x: self.adaptivemaxpool3d_4(self.convdropout(self.relu(self.conv4(x)))), out)
        # out = checkpoint(lambda x: self.adaptivemaxpool3d_5(self.convdropout(self.relu(self.conv5(x)))), out)
        out = self.conv1(x)
        out = self.relu(out)
        out = self.convdropout(out)
        out = self.adaptivemaxpool3d_1(out)
        ...
        out = self.relu(out)
        out = self.convdropout(out)
        out = self.adaptivemaxpool3d_5(out)
...
class CNNMINE3X_dropout(nn.Module):
    def __init__(self, n, k, w1=8, w2=4, w3=2, dropoutfc=None, dropoutconv=None, input_size=1, initializations=None):
        super().__init__()
        self.conv1 = nn.Conv3d(input_size, n, kernel_size=4, stride=1)
        self.conv2 = nn.Conv3d(n, int(k*n), kernel_size=3, stride=1)
        self.conv3 = nn.Conv3d(int(k*n), int(n*k*k), kernel_size=3, stride=1)
        self.fc1 = nn.Linear(int(n*k*k), int(k*n))
        self.fc2 = nn.Linear(int(k*n), n)
        self.fc3 = nn.Linear(n, 1)
        self.relu = nn.LeakyReLU()
        self.adaptivemaxpool3d_3 = nn.AdaptiveMaxPool3d(w1)
        self.adaptivemaxpool3d_2 = nn.AdaptiveMaxPool3d(w2)
        self.adaptivemaxpool3d_1 = nn.AdaptiveMaxPool3d(w3)
        self.fcdropout = nn.Dropout(p=dropoutfc)
        self.convdropout = nn.Dropout3d(p=dropoutconv)
...
class CNNMINE3X(nn.Module):
    def __init__(self, n, k, w1=12, w2=6, input_size=1, initializations=None):
        super().__init__()
        self.conv1 = nn.Conv3d(input_size, n, kernel_size=4, stride=1)
        self.conv2 = nn.Conv3d(n, int(k*n), kernel_size=4, stride=1)
        self.conv3 = nn.Conv3d(int(k*n), int(n*k*k), kernel_size=3, stride=1)
        self.fc1 = nn.Linear(int(n*k*k), int(k*n))
        self.fc2 = nn.Linear(int(k*n), n)
        self.fc3 = nn.Linear(n, 1)
...
class CNNMINE3(nn.Module):
    def __init__(self, input_size=1, initializations=None):
        super().__init__()
        self.conv1 = nn.Conv3d(input_size, 16, kernel_size=4, stride=1)
        ...
        self.adaptivemaxpool3d_3 = nn.AdaptiveAvgPool3d(6)
        self.adaptivemaxpool3d_2 = nn.AdaptiveAvgPool3d(3)
        self.adaptivemaxpool3d_1 = nn.AdaptiveAvgPool3d(1)
